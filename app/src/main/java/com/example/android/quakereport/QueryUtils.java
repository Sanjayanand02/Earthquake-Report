...
JSONArray earthaarray= start.getJSONArray("features");
for (int i=0;i<earthaarray.length();i++)
{
    JSONObject arrayloop=earthaarray.getJSONObject(i);
    JSONObject properties =arrayloop.getJSONObject("properties");
    double magnitude = properties.getDouble("mag");
    String location = properties.getString("place");
    long time = properties.getLong("time");
    Date currentdate=new Date(time);
    SimpleDateFormat dateFormat= new SimpleDateFormat("MMM DD, yyyy");
    String displaydate=dateFormat.format(time);
    // Extract the value for the key called "url"
    String url = properties.getString("url");

   Earthquake nearthquake=new Earthquake(magnitude,location,time,url);
